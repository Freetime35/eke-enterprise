"""Tests for import-job result item inspection."""
...
def test_get_result_items_with_filter() -> None:
    ...
    items = service.get_result_items(
        job.job_uuid,
        item_status=EurLexBulkImportStatus.FAILED,
    )
...
def test_get_result_items_rejects_pending_job() -> None:
    service, repository = make_service()
    ...
    with pytest.raises(
        ...
        service.get_result_items(job.job_uuid)
